...
float FenerlSchilick(float NdotV, float r0) {
    // t=(1-NdotV)^5
    float t = (1 - NdotV);
    t = t*t*t*t*t;
    return r0 + (1 - r0) * t;
...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
	...
	for (int i = 0; i < sample_count; i++) {
		...
		Vec3f H = normalize(V + wi);

        float NdotWi = std::max(dot(N,wi), 0.0f);
        float fr = 
            FenerlSchilick(NdotV,0.8f)
            * DistributionGGX(N,H,roughness)
            * GeometrySmith(roughness, NdotV ,NdotWi)
            /4.0f/NdotWi/NdotV;
        float pdf = 1.0f/PI;
        A += fr / pdf;
        B += fr / pdf;
